...
async def list_food_items(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    result = await db.execute(select(FoodItem).where(FoodItem.user_id == user.id).order_by(FoodItem.name))
    return result.scalars().all()


@food_router.post("", response_model=FoodItemOut, status_code=201)
async def create_food_item(data: FoodItemCreate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    item = FoodItem(id=uuid.uuid4(), user_id=user.id, **data.model_dump(by_alias=False))
    db.add(item)
    await db.commit()
...
async def update_food_item(
    item_id: uuid.UUID, data: FoodItemUpdate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
):
    result = await db.execute(select(FoodItem).where(FoodItem.id == item_id, FoodItem.user_id == user.id))
...
async def delete_food_item(item_id: uuid.UUID, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    result = await db.execute(select(FoodItem).where(FoodItem.id == item_id, FoodItem.user_id == user.id))
    item = result.scalars().first()
    if not item:
        raise HTTPException(404, "Food item not found")
...
async def list_recipes(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    result = await db.execute(select(Recipe).where(Recipe.user_id == user.id).order_by(Recipe.name))
    return result.scalars().all()


@recipe_router.post("", response_model=RecipeOut, status_code=201)
async def create_recipe(data: RecipeCreate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    recipe = Recipe(
        id=uuid.uuid4(),
        user_id=user.id,
        name=data.name,
...
async def update_recipe(
    recipe_id: uuid.UUID, data: RecipeUpdate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
):
...
async def delete_recipe(recipe_id: uuid.UUID, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    result = await db.execute(select(Recipe).where(Recipe.id == recipe_id, Recipe.user_id == user.id))
    recipe = result.scalars().first()
    if not recipe:
        raise HTTPException(404, "Recipe not found")
