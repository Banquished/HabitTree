...
async def get_active_mission(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Mission).where(Mission.status == "active").order_by(Mission.created_at.desc()))
...
async def create_mission(data: MissionCreate, db: AsyncSession = Depends(get_db)):
    # Check for date overlap with non-abandoned missions
...
async def update_mission(mission_id: uuid.UUID, data: MissionUpdate, db: AsyncSession = Depends(get_db)):
